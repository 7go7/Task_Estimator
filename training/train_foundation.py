...
for i in range(len(df)):
    text_vector = vectorize_text_hashed(df.iloc[i]['desc'])
    # Фіксований вектор: [priority, role, hash_0, hash_1, ..., hash_8191]
    features = [df.iloc[i]['prio'], df.iloc[i]['role']] + text_vector
    X_base.append(features)
    
y_base = df['hours'].values

print("Тренування Фундаментальної моделі (Hashing Trick)...")
