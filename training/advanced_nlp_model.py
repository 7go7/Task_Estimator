...
# 1. Розширений набір сирих даних (Чим більше різного тексту - тим краще)
raw_data = [
    # Junior (Role: 0)
...
print("1. Аналіз тексту та витягування семантики...")
# Збираємо всі описи докупи
descriptions = [item["desc"] for item in raw_data]

# Налаштовуємо TfidfVectorizer. 
# Він сам викине зайві слова (the, a, on) і знайде найважливіші терміни
vectorizer = TfidfVectorizer(
    max_features=500,
    stop_words="english",
    use_idf=False,   # IMPORTANT: disables IDF
    norm=None        # IMPORTANT: disables normalization
)
X_text_features = vectorizer.fit_transform(descriptions).toarray()

# Отримуємо словник найважливіших слів, які "вивчила" модель
vocabulary = vectorizer.get_feature_names_out().tolist()
print(f"Ключові слова для моделі: {vocabulary}")

# 2. Формуємо фінальну матрицю ознак X
# Вектор матиме вигляд: [priority, role, word1_score, word2_score, ..., word10_score]
X_train = []
y_train = []

for i, item in enumerate(raw_data):
    # Беремо пріоритет та роль
    base_features = [item["priority"], item["role"]]
    # Додаємо семантичні ознаки тексту (TF-IDF оцінки слів)
...
# 3. Навчання моделі
print("2. Тренування XGBoost з урахуванням семантики...")
model = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, max_depth=4)
model.fit(X_train, y_train)

# 4. Збереження словника та моделі для C++
model_dir = '../backend/build/model'
os.makedirs(model_dir, exist_ok=True)

# Зберігаємо модель
model.save_model(os.path.join(model_dir, 'task_estimation_model.json'))

# Зберігаємо СЛОВНИК (це дуже важливо для C++)
with open(os.path.join(model_dir, 'vocabulary.json'), 'w') as f:
    json.dump(vocabulary, f)

print(f"Готово! Словник і модель збережено у {model_dir}")